...
def turnint(string):
        ...
        while (flag==False):
                try:
                        intvara=int(strtoint)
                except:
                        strtoint=raw_input("Please Re-type\n")
                else:
                        flag=True
        return intvara
...
def execute():
        ...
        if (fileCount==0):
                ...
                for f in userroutingFileLists:
                        ...
                        print(str(findex)+" "+f)
                selectFile=raw_input("Please input the index of the user routing file, 0 for none\n")
        intselectFile=turnint(selectFile)
        if (intselectFile<=0):
                selectFile="0"
        if (intselectFile>=fileCount):
                selectFile="0"
        if (selectFile=="0"):
